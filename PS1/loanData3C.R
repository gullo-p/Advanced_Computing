...
boundary_12 <-   data.frame(solvency=x, PIratio=y12, 
                            deny=rep("Boundary_app/deny", length(x)))  ##Boundary between Approved and Denied
...
boundary_13 <- data.frame(solvency=x, PIratio=y13, 
                          deny=rep("Boundary_app/undecided", length(x))) ##Boundary between Approved and Undecided
...
boundary_23 <- data.frame(solvency=x, PIratio=y23, 
                          deny=rep("Boundary_deny/undecided", length(x))) ##Boundary between Denied and Undecided
